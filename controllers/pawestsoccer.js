...
let INPUT_FILE = '';
let INPUT_URL = 'https://www.pawest-soccer.org/clubs/club-directory/';
let OUTPUT_FILE = 'data/output/pawestsoccer-urls.csv';
let csvRecords = [17101, 15201, 18601, 18101, 17001];
...
exports.pawestsoccer = async (req, res, next) => {
    try {
        console.log('pawestsoccer');
        await init();
        res.send({msg: 'ok'});
      } catch (error) {
        console.error('there was an error');
